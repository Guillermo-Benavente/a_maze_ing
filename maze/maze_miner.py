...
class MazeMiner():
    ...
    def _mined(self):
        self.valid_cells: set[Cell] = self.maze.valid_cells
        self.mined_cells: list[list[Cell]] = []
        miners: int = int(
            (self.maze.data.WIDTH * self.maze.data.HEIGHT) * 0.04
        )
    ...
    def _inital_points(self, miners: int):
    ...
    def _maze_mining(self, miners: int):
        maze: list[list[Cell]] = self.maze.maze
        for miner in range(miners):
                ...
                for _ in range(num_mined):
                    ...
                    if (
                        LimitWallType.NORTH not in cell_mined.limit_wall_type and
                        openings & 8
                    ):
                        new_cell = maze[y - 1][x]
                    ...
                    if (
                        LimitWallType.EAST not in cell_mined.limit_wall_type and
                        openings & 4
                    ):
                        new_cell = maze[y][x + 1]
                        if self._try_cell_fusion(new_cell, miner):
                    ...
                    if (
                        LimitWallType.SOUTH not in cell_mined.limit_wall_type and
                        openings & 2
                    ):
                        new_cell = maze[y + 1][x]
                        if self._try_cell_fusion(new_cell, miner):
                    ...
                    if (
                        LimitWallType.WEST not in cell_mined.limit_wall_type and
                        openings & 1
                    ):
                        new_cell = maze[y][x - 1]
                        if self._try_cell_fusion(new_cell, miner):
